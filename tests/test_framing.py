...
def test_cmd0_never_completes_before_the_numeric_block(cmd0_capture: Capture) -> None:
    ...
    for length in range(len(cmd0_capture.raw)):
        ...
        if is_complete(prefix, "0"):
            ...
            assert b"\r" in prefix[MIN_CMD0_LEN:]
...
def test_cmd0_checksum_byte_may_be_cr_or_lf(cmd0_capture: Capture, checksum: int) -> None:
    """The documented reason cmd `0` is length-gated instead of CR-scanned.

    The checksum at offset 57 can legitimately be any byte 0-255. If framing
    scanned for the first CR, a 0x0D checksum would truncate the frame right
    before the type string — roughly 1 frame in 256.
    """
